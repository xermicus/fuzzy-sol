...
contract ComboOracle_UniV2_UniV3 is Owned {
    ...
    // UniV2 / SLP Info
    function uniV2LPInfo(address pair_address) public view returns (UniV2PairInfo memory) {
    ...
    function uniV2LPPricePack(address lp_token_address) public view returns (
        ...
        UniV2PairInfo memory lp_info_pack = uniV2LPInfo(lp_token_address);

        // Get the price for token0 from the ComboOracle
        // Will revert if ComboOracle doesn't have a price for token0
        (uint256 token0_precise_price, ) = combo_oracle.getTokenPrice(lp_info_pack.token0);

        // Prepare division
        {
            // PRECISE_PRICE_PRECISION's cancel out
            uint256 tmp_numerator = lp_info_pack.token0_reserves * token0_precise_price * lp_info_pack.token0_missing_decimals_multiplier;
            uint256 tmp_denominator = lp_info_pack.lp_total_supply;
        ...
        // Set the LP token symbol and name
        token_symbol = lp_info_pack.token_symbol;
        token_name = lp_info_pack.token_name;
    }
    ...
    function getUniV3NFTInfo(uint256 token_id) public view returns (UniV3NFTInfo memory) {
        UniV3NFTBasicInfo memory basic_info = getUniV3NFTBasicInfo(token_id);

        // Get pool price info
        uint160 sqrtPriceX96;
        {
            address pool_address = univ3_factory.getPool(basic_info.token0, basic_info.token1, basic_info.fee);
        ...
        uint256 token1_val_usd = 0; 
        {
            ...
            uint160 sqrtRatioAX96 = TickMath.getSqrtRatioAtTick(basic_info.tickLower);
            uint160 sqrtRatioBX96 = TickMath.getSqrtRatioAtTick(basic_info.tickUpper);

            // Get amount of each token for 0.1% liquidity movement in each direction (1 per mille)
            uint256 liq_pricing_divisor = 1000;
            (uint256 token0_1pm_amt, uint256 token1_1pm_amt) = LiquidityAmounts.getAmountsForLiquidity(sqrtPriceX96, sqrtRatioAX96, sqrtRatioBX96, uint128(basic_info.liquidity / liq_pricing_divisor));

            // Get missing decimals
            uint256 token0_miss_dec_mult = 10 ** (uint(18) - ERC20(basic_info.token0).decimals());
            uint256 token1_miss_dec_mult = 10 ** (uint(18) - ERC20(basic_info.token1).decimals());

            // Get token prices
            // Will revert if ComboOracle doesn't have a price for both token0 and token1
            (uint256 token0_precise_price, ) = combo_oracle.getTokenPrice(basic_info.token0);
            (uint256 token1_precise_price, ) = combo_oracle.getTokenPrice(basic_info.token1);
        ...
        return UniV3NFTInfo(
            ...
            token1_val_usd,
            nft_ttl_val,
            ERC20(basic_info.token0).symbol(),
            ERC20(basic_info.token1).symbol(),
            (uint256(basic_info.liquidity) * PRECISE_PRICE_PRECISION) / nft_ttl_val
        );
    }
