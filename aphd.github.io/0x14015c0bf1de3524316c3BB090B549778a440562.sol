...
contract NFT is ERC721Enumerable, Ownable {
  ...
  uint256 public limtMintTime = 0 minutes;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        ...
        if(ownerMintedCount >= 1 && limtMintTime > 0){
            uint256 mins = (limtMintTime/60);
            uint256 checkTimeLimit = (block.timestamp - addressLastMint[msg.sender]);
            require( checkTimeLimit > limtMintTime,string(abi.encodePacked("Limit Mint ",mins.toString()," minutes")));
        }
  ...
  function giveaway(address _to, uint256 _mintAmount) public payable onlyOwner{
    uint256 supply = totalSupply();
  ...
  function setlimtMintTime(uint256 _newLimit) public onlyOwner() {
    limtMintTime = _newLimit;
  }
