...
// This contract handles "serving up" rewards for xDint holders by trading tokens collected from fees for Dint.
contract DintMaker is Ownable {
    using SafeMath for uint;
    using SafeERC20 for IERC20;
    IUniswapV2Router02 public routerv2;
    IUniswapV2Factory public factory;
    address public dintSafe;
    address public dint;
    address public weth;
    ...
    function removeLiqidity(address token0, address token1, uint amount) public {
        require(operators[msg.sender], 'DintMaker: no permistion');
    ...
    function marketBuyDintWithETH(address[] calldata path, uint amount, uint deadline) public {
      require(operators[msg.sender], 'DintMaker: no permistion');
      require(path[path.length - 1] == dint, 'DintMaker: wrong path');
      for (uint i = 1; i < path.length - 1; i++) {
        require(acceptTokens[path[i]], 'DintMaker: wrong path');
      }
    ...
    function marketBuyDintWithToken(address[] calldata path, uint amount, uint deadline) public {
      require(operators[msg.sender], 'DintMaker: no permistion');
      address token = path[0];
      if (token == dint) {
          IERC20(token).safeTransfer(dintSafe, amount);
      } else  {
        require(path[path.length - 1] == dint, 'DintMaker: wrong path');
        for (uint i = 1; i < path.length - 1; i++) {
          require(acceptTokens[path[i]], 'DintMaker: wrong path');
    ...
    function convert(address token0, address token1) public {
        ...
        if (IERC20(weth).balanceOf(address(this)) > 0) {
          ...
          _swap(weth, dint, IERC20(weth).balanceOf(address(this)));
        }

        if (IERC20(dint).balanceOf(address(this)) > 0) {
          IERC20(dint).safeTransfer(dintSafe, IERC20(dint).balanceOf(address(this)));
        }
    }
    ...
    function _toBaseToken(address token, uint amountIn) internal {
      if (token == dint) return;

      if (_canSwap(token, dint)) {
        _swap(token, dint, amountIn);
      }
